...
const SidebarItem = ({ to, icon: Icon, label }: { to: string; icon: any; label: string }) => {
    ...
    return (
        <Link
            to={to}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-[13px] font-medium transition-colors ${isActive
        ...
        <aside className="w-64 border-r border-slate-200 flex flex-col bg-slate-50/50 backdrop-blur-xl h-screen sticky top-0">
            <div className="h-14 flex items-center px-6 border-b border-slate-200/50">
                <div className="flex items-center gap-2.5">
                    ...
                    <h1 className="text-sm font-bold tracking-tight text-slate-900">InventoryPro</h1>
                </div>
            </div>

            <div className="p-4 flex-1 space-y-8 overflow-y-auto">
                <section>
                    <h3 className="px-3 text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-3">Workspace</h3>
                ...
                <section>
                    <h3 className="px-3 text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-3">Inventory</h3>
                    <nav className="space-y-1">
                ...
                <section>
                    <h3 className="px-3 text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-3">Manufacturing</h3>
                    <nav className="space-y-1">
                        <SidebarItem to="/manufacturing/definitions" icon={Settings2} label="Definitions" />
                        <SidebarItem to="/manufacturing" icon={Activity} label="Job Orders" />
                        <SidebarItem to="/inventory/ledger" icon={ListIcon} label="Transaction Log" />
                    </nav>
                </section>

                <section>
                    <h3 className="px-3 text-[12px] font-bold text-slate-400 uppercase tracking-widest mb-3">Network</h3>
                    <nav className="space-y-1">
                        <SidebarItem to="/network/brokers" icon={Users} label="Brokers" />
                    </nav>
                </section>
            </div>
