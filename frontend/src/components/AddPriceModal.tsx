...
import { productsApi, PriceTypeLabel } from '../api/productsApi';
import type { PriceType } from '../api/productsApi';
...
export const AddPriceModal: React.FC<AddPriceModalProps> = ({ productId, onSuccess, onCancel }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        uiLoader.show();
        try {
            await productsApi.addProductPrice(productId, {
            ...
            uiAlert.success("Price added successfully");
            onSuccess();
        } catch (error) {
            uiAlert.error("Failed to add price");
        } finally {
            uiLoader.hide();
        }
    };
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-300">
                <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        <h3 className="text-xl font-bold text-slate-900 tracking-tight">Set Product Price</h3>
                        <p className="text-sm text-slate-400 font-medium">Define a new pricing strategy.</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-6">
                    <div>
                        <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Price Type</label>
                        <select
                            value={formData.priceType}
                            onChange={(e) => setFormData({ ...formData, priceType: Number(e.target.value) })}
                        ...
                        >
                            {Object.entries(PriceTypeLabel).map(([key, label]) => (
