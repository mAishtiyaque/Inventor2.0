    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h2 className="text-3xl font-black text-slate-900 tracking-tight">
                Job #{execution.id.split("-")[0].toUpperCase()}
              </h2>
              <span
                className={cn(
            ...
            <p className="text-slate-600 font-medium flex items-center gap-1">
              ...
              {execution.processDefinitionVersion?.id && (
                <span className="text-xs text-slate-500">
                  V{execution.processDefinitionVersion.versionNumber}
                </span>
              )}
            </p>
            <p className="text-[10px] text-slate-900">
              {execution.processDefinitionVersion?.notes ||
                "No Notes Available"}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          trend={
            isCompleted
              ? execution.actualOutputQty >= execution.plannedQty
                ? "positive"
                : "negative"
              : undefined
          }
        ...
        <MetricCard
          ...
          label="Efficiency"
          value={
            execution.plannedQty > 0
              ? Math.round(
                  (execution.actualOutputQty / execution.plannedQty) * 100,
                )
              : 0
          }
          unit="%"
        ...
        <MetricCard
          ...
          value={
            execution.totalCost?.toLocaleString("en-IN", {
              style: "currency",
              currency: "INR",
            }) || "₹0"
          }
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50">
                    ...
                    <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">
                      Planned Qty
                    </th>
                    <th className="px-4 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">
                      Actual Qty
                    ...
                    <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-right">
                      Actual Total Cost
                    </th>
                  </tr>
                </thead>
                      ...
                      >
                        <td className="px-8 py-4">
                          <p className="font-bold text-slate-900">
                            {io.product?.name || "Unknown Product"}
                          </p>
                        ...
                        <td className="px-4 py-4 text-right font-bold text-slate-400 text-xs">
                          {isCompleted
                            ? `${io.actualQty} x ₹${io.unitCost.toFixed(2)}`
                            : "-"}
                        </td>
                        <td className="px-8 py-4 text-right font-black text-slate-900 scale-95 origin-right">
                          {isCompleted
                            ? `₹${io.actualCost?.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`
                            : "-"}
                        </td>
                      </tr>
                    ))}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-200">
                    {execution.vendor && execution.vendor.name ? (
                      <Link
                        to={`/vendor/brokers/${execution.vendor.id}`}
                        className="underline hover:text-indigo-200"
                      >
                        {execution.vendor.name}
                      </Link>
                    ) : (
                      (execution.vendorId && execution.vendorId) ||
                      "Internal Production"
                    )}
                  </p>
                </div>
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-200">
                    {execution.processDefinitionVersion?.id
                      ? `Version ${execution.processDefinitionVersion.versionNumber}`
                      : "Default"}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-3xl p-6 flex gap-4">
              ...
              <div>
                ...
                <p className="text-blue-700 text-xs leading-relaxed">
                  This job is currently active. Use the production floor to
                  update status or record scrap.
                </p>
              </div>
            </div>
          )}
...
const MetricCard: React.FC<{
  ...
  <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
    ...
    <div className="flex items-baseline gap-1">
      <span className={cn("text-2xl font-black text-slate-900", color)}>
        {value}
      </span>
